...
export const api = {
  ...
  mailboxes: {
    list: () => request<any[]>('/mailboxes'),
    get: (id: string) => request<any>(`/mailboxes/${id}`),
    getBySlug: (slug: string) => request<any>(`/mailboxes/by-slug/${slug}`),
    create: (data: { name: string }) => request<any>('/mailboxes', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: string, data: any) => request<void>(`/mailboxes/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
    delete: (id: string) => request<void>(`/mailboxes/${id}`, { method: 'DELETE' }),
    fetch: (id: string) => request<any>(`/mailboxes/${id}/fetch`, { method: 'POST' }),
    listIMAPFolders: (config: any) => request<any[]>(`/mailboxes/list-imap-folders`, { method: 'POST', body: JSON.stringify(config) }),
  },
  email: {
